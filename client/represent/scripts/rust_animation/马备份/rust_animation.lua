...
tRideParams = 
{
	...
	isRust = "isRust", --是否为Rust状态
	RunFDirectionSwitch = "RunFDirectionSwitch", --坐骑前进方向切换
}
RustInstances = {}
IsRustState = 0
--变量
...
function CheckRustState(IsRustState, CharacterObj)
    local upid, downid = CharacterObj.GetAnimationBodyState()
    local isLinking = CharacterObj.IsLinking()
    if (upid == ANIMATION_UP_TYPE.UNKNOWN or upid == ANIMATION_UP_TYPE.NONE) or isLinking == false then
        IsRustState = 1
    else
        IsRustState = 0
    end
...
function UpdateCharacter(dwRustID,CharacterID)
	...
	Instance.isRust = CheckRustState(IsRustState, CharacterObj)
	...
	if not frameData.bOnRide then
		...
		Instance.lastHorizonSpeed = Instance.HorizonSpeed 
		Instance.lastHorizonVelocity = Instance.HorizonVelocity
		Instance.lastHorizonAcceleration = Instance.HorizonAcceleration
		Instance.tPreTransform= Instance.CurrentTransform
...
function UpdateRide(dwRustID,CharacterID)
	...
	print(string.format("Instance.DirectionOffset: %f", Instance.DirectionOffset))
	...
	RunFDirectionSwitch = 0

	Instance.DirectionOffset = PlayerLogicOffset - InputLogicOffset --输入方向和角色逻辑方向的偏移角度
	if Instance.DirectionOffset < -15 then
		RunFDirectionSwitch = 0 --左转
	elseif Instance.DirectionOffset >= -15 and Instance.DirectionOffset <= 15 then
		RunFDirectionSwitch = 1 --前进
	elseif Instance.DirectionOffset > 15 then
		RunFDirectionSwitch = 2 --右转
	end
	...
	RustAnimationSetParameter(dwRustID, RUST_ANIMATION_PARAMETER_TYPE.I, tRideParams.RunFDirectionSwitch, RunFDirectionSwitch) --坐骑前进方向切换
...
function OnUpdate(eType, dwRustID, dwCharacterID)
	...
	if Instance.objectType == RUST_ANIMATION_OBJECT_TYPE.CHARACTER then	
		UpdateCharacter(dwRustID,dwCharacterID)
	end
	if Instance.objectType == RUST_ANIMATION_OBJECT_TYPE.RIDE then
		if frameData.bOnRide then
			UpdateRide(dwRustID,dwCharacterID)
		end
	end
end
